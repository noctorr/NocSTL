 ...
 * @note This will strictly use GCC's compiler intristics which will not be
 * supported on other C++ Compilers so do not use this - if you are not using
 * the GNU Compiler for you're C++ project.
 * */
...
#define _STANDARD_TEMPLATED_LIBARY_SIMD_INTRISTICS_DATA_TYPES 1
...
#ifdef _SIOUD_ABLE_TO_USING
using I32MD64 = std::uint32_t [[gnu::aligned(4), gnu::vector_size(8)]];
using I32MD128 = std::uint32_t [[gnu::aligned(4), gnu::vector_size(16)]];
#else
typedef std::uint32_t I32MD64 [[gnu::aligned(4), gnu::vector_size(8)]];
typedef std::uint32_t I32MD128 [[gnu::aligned(4), gnu::vector_size(16)]];
#endif

// 64-Bit Signed Integers
#ifdef _SIOUD_ABLE_TO_USING
using I64MD128 = std::uint64_t [[gnu::aligned(8), gnu::vector_size(16)]];
using I64MD256 = std::uint64_t [[gnu::aligned(8), gnu::vector_size(32)]];
#else
typedef std::uint64_t I64MD128 [[gnu::aligned(8), gnu::vector_size(16)]];
...
add_fmd64(FMD64 vector1, FMD64 vector2) {
